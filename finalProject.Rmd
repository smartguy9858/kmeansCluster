...
salesDF$perItem = salesDF$Val/(salesDF$Quant+1)
...
```{r echo=FALSE}
sortIndexQ = sort(fraud_ok$Quant, index.return=TRUE, decreasing = TRUE)
print('highest quantity sales')
print('Person ID      quantity    tyep')
for (i in 1:20) print(paste(fraud_ok$ID[sortIndexQ$ix][i], fraud_ok$Quant[sortIndexQ$ix][i], fraud_ok$Insp[sortIndexQ$ix][i]))
#print(fraud_ok$ID[sortIndex$ix][1:20])
sortIndexV = sort(fraud_ok$perItem, index.return=TRUE, decreasing = TRUE)
print('------------------------')
print('highest price items')
print('Person ID    item price    transaction')
for (i in 1:20) print(paste(fraud_ok$ID[sortIndexV$ix][i],"   ",round(10^(fraud_ok$perItem[sortIndexV$ix][i]),4), fraud_ok$Insp[sortIndexV$ix][i]))
